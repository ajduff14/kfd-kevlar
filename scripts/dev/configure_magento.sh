...
if [ $# -ne 14 ]; then
    echo $0: usage: install_magento.sh dbhost dbuser dbpassword dbname cname adminfirstname adminlastname adminemail adminuser adminpassword magentourl magentolanguage magentocurrency magentotimezone
    exit 1
fi
...
[ -f $app_archive ] && echo "Media file Found" || echo "Media file Not found"
 
sudo tar xzfP ~/magento.tar.gz --directory /var/www/html
...
./magento setup:install \
  ...
  --language=$magentolanguage \
  --currency=$magentocurrency \
  --timezone=$magentotimezone \
  --use-secure-admin=1 --use-secure=1 --base-url-secure=https://$cname/
